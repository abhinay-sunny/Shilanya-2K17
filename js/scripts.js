(function ($) {
	...
	$('#tweets').twittie({
		username: 'envato',
		count: 5,
		template: '<div class="tweet">{{tweet}}</div><div class="follow">Follow <a href="https://twitter.com/envato">@{{user_name}}</a> and stay posted!</div>',
		apiPath: './scripts/Tweetie/api/tweet.php'
	}, function() {
		$('#tweets ul').owlCarousel({
			loop: true,
			nav: false,
			dots: false,
			items: 1,
	...
	setTimeout(function() {
		var str = $('.video-slider-wrapper').text();
		// alert(str);
		$(':contains("Ninja Slider trial version")').each(function(){
			$(this).html($(this).html().split("Ninja Slider trial version").join(""));
		});
	}, 1000);
	setTimeout(function() {
		var str = $('.video-slider-wrapper').text();
